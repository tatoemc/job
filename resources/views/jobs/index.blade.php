...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الوظائف </h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                     كل الوظائف </span>
            </div>
        </div>
                ...
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        @can('isAdmin')
                            ...
                            data-toggle="modal" href="#modaldemo8">أضافة وظيفة</a>
                            @endcan
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'>
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">تاريخ اضافة الوظيفة</th>
                                    @can('isAdmin')
                                ...
                                @if ($jobs->count() > 0)
                                    @foreach ($jobs as $index => $job)
                                        <tr>
                                            ...
                                            <td>{{ $job->name }} </td>
                                            <td>{{ $job->desc }}</td>
                                            <td>{{ date( 'j m Y', strtotime($job->created_at)) }}
                                            <td>
                                                <a href="{{ route('jobs.show', $job->id) }}" class="btn btn-sm btn-success"
                                                    title="عرض"><i class="las la-eye"></i></a>
                                                @can('isAdmin')
                                                <a class="btn btn-sm btn-info"  href="{{ route('jobs.edit',$job->id)}}" title="تعديل"><i
                                                    class="las la-pen"></i></a>

                                                <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"
                                                data-id="{{ $job->id }}" data-name="{{ $job->name }}"
                                                data-toggle="modal" href="#delete_job" title="حذف"><i
...
<!-- End Basic modal -->
    ...
    <!-- حذف -->
    <div class="modal fade" id="delete_job" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form action="{{ route('jobs.destroy', 'delete') }}" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
                </div>
